...
### Changed
- All three packages: bumped the ProcessKit dependency to 1.3.2 (was 1.2.0). ProcessKit now always closes a command's stdin at start when no input is piped (removing a latent hang where a command that unexpectedly read stdin could block waiting for input that never arrives), and `RunRawAsync` returns stdout captured faithfully — the process's exact line endings and any trailing newline are preserved.
