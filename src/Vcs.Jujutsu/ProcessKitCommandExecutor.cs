...
/// surface — callers see only <see cref="JjCommandResult"/>.
/// </summary>
internal sealed class ProcessKitCommandExecutor : ICommandExecutor
{
	...
	public async Task<JjCommandResult> RunAsync(IReadOnlyList<string> arguments, TimeSpan? timeout, string? standardInput, CancellationToken cancellationToken)
	{
		...
		catch (Win32Exception ex)
		{
			...
			throw new JujutsuCliException(-1, ex.Message, joined, false,
				$"Could not start `{_executable}`: {ex.Message}", ex);
		}

		return new JjCommandResult(result.StdOut, result.StdErr, result.ExitCode) { WasTimedOut = result.WasTimedOut };
	}
}
